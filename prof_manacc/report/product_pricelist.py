...
class product_pricelist_prof_manacc(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(product_pricelist_prof_manacc, self).__init__(cr, uid, name, context=context)
        self.pricelist={}
    ...
    def _get_categories(self, products, data):
        ...
        if not cats:
            return res
        for cat in [1]:
            product_ids=self.pool.get('product.product').search(self.cr, self.uid, [('id', 'in', pro_ids)],order='default_code,id',context=self.localcontext)
            products = []
            for product in self.pool.get('product.product').read(self.cr, self.uid, product_ids, ['image','attribute_value_ids', 'code','name','virtual_available'], context=self.localcontext):
                ...
                products.append(val)
            res.append({'name':'Tutte','products': products})
        return res
...
class report_product_pricelist_prof_manacc(osv.AbstractModel):
    _name = 'report.prof_manacc.report_pricelist_prof_manacc'
    _inherit = 'report.abstract_report'
    _template = 'prof_manacc.report_pricelist_prof_manacc'
    _wrapped_report_class = product_pricelist_prof_manacc
