...
class product_pricelist_wizard(osv.osv_memory):

    _name = 'product.pricelist.wizard'
    _columns = {
        'name':fields.char('Name', size=64, required=False, readonly=False),
        'perc_ric': fields.property(type = 'float', digits_compute=dp.get_precision('Product Price'), 
                                          
                                          groups="base.group_user", string="percentuale di ricarica "),
    }
    _defaults = {
    'name':'update price_surcharge',
    }

    def pricelist_ricarica(self, cr, uid, ids,context=None):
        if hasattr(ids, '_iter_'):
                ids=ids
        else:
                ids=[ids]
        this=self.browse(cr, uid, ids[0], context=context)
        perc_ric=this.perc_ric
        if context.get('active_ids', []):
            versionlist_ids = self.pool.get('product.pricelist.version').search(cr, uid, [('pricelist_id','in',context.get('active_ids', []))],context=context)
            item_ids= self.pool.get('product.pricelist.item').search(cr, uid, [('price_version_id','in',versionlist_ids)],context=context)
            for pricelist_id_obj in self.pool.get('product.pricelist.item').browse(cr,uid,item_ids,context=context):
                self.pool.get('product.pricelist.item').write(cr, uid,pricelist_id_obj.id,{'price_surcharge':pricelist_id_obj.price_surcharge*(100+perc_ric)/100},context=context)
        return {}


class product_manacc(osv.osv_memory):
    _name = 'product.product.standard_price'
    _description = 'update standard_price'

    _columns = {
        'name':fields.char('Name', size=64, required=False, readonly=False),
        'product_tmpl_id':fields.many2one('product.template', 'Modello', required=False), 
        'attribute_value_id':fields.many2one('product.attribute.value', 'Valore Attributo', required=False), 
        'standard_price': fields.property(type = 'float', digits_compute=dp.get_precision('Product Price'), 
                                          help="Cost price of the product template used for standard stock valuation in accounting and used as a base price on purchase orders. "
                                               "Expressed in the default unit of measure of the product.",
                                          groups="base.group_user", string="Cost Price"),
    }
    def update_standard_price(self, cr, uid, ids, context=None):
            if hasattr(ids, '_iter_'):
                ids=ids
            else:
                ids=[ids]
            ...
            if this.product_tmpl_id:
                    ids=[this.product_tmpl_id.id]
                    variants_obj=templ_obj.browse(cr,uid,ids,context=context).product_variant_ids
            else:
                    ids=[]
            ...
            if ids==[]:
                if context.get('active_ids', []):
                    ...
                    variants_obj=product_obj.browse(cr,uid,ids,context=context)
            if  ids==[]:                   
                raise except_orm(_('Nessuna scelta è Stata Fatta'), 'Selezionare almeno una distinta base')
            
            F_process=False
            """ leggo la distinta """
            for  product_id_obj in variants_obj:
                        if attribute_value_id:
                            for var_attribute_value_id in product_id_obj.attribute_value_ids:
                                if var_attribute_value_id==attribute_value_id:
                                    product_obj.write(cr,uid,product_id_obj.id,{'standard_price':standard_price},context=context)
                                    F_process=True
                                    break
                        else:
                                product_obj.write(cr,uid,product_id_obj.id,{'standard_price':standard_price},context=context)
            ...
            if F_process==False:
                    raise Warning(_('Attenzione Nessuna variante è sta generata'))
                    return False
            return True

    def update_standard_price_all(self, cr, uid, ids, context=None):
            if hasattr(ids, '_iter_'):
                ids=ids
            else:
                ids=[ids]
            ...
            product_obj=self.pool.get('product.product')
            templ_obj=self.pool.get('product.template')
            prod_ids=product_obj.search(cr,uid,[('active','=',True),('standard_price','=',0)],context=context)
            for  product_id_obj in product_obj.browse(cr,uid,prod_ids,context=context):
                    if product_id_obj.standard_price==0 or product_id_obj.standard_price==None:
                            product_obj.write(cr,uid,product_id_obj.id,{'standard_price':product_id_obj.product_tmpl_id.standard_price},context=context)
            return True
