...
class product_price_list_prof_manacc(osv.osv_memory):
    _name = 'product.price_list.prof_manacc'
    ...
    def print_report(self, cr, uid, ids, context=None):
            ...
            if context.get('active_ids', []):
                ...
                pricelist_id=this.price_list_pubblico.id
                pricelist = self.pool.get('product.pricelist').browse(cr, uid, pricelist_id, context=context)
                version_id=pricelist.version_id
                prod_id_items=[]
                for item in pricelist.version_id.items_id:
                    if item.product_id:
                        prod_id_items.append(item.product_id.id)
                if  prod_id_items:
                           prod_ids=prod_id_items
                for prod_id in prod_ids:
                    prod_id_obj=product_obj.browse(cr,uid,context=context)
                    price_dict = self.pool.get('product.pricelist').price_get(cr, uid, [pricelist_id], prod_id, 0.0,partner=None, context=context)
                    if price_dict[pricelist_id]:
                        print price_dict
                        prod_ids_fin.append(prod_id)
                datas = {'select':False,'ids': prod_ids_fin}
            ...
            datas['form'] = res
            print datas
            return self.pool['report'].get_action(cr, uid, [], 'prof_manacc.report_pricelist_prof_manacc', data=datas, context=context)
