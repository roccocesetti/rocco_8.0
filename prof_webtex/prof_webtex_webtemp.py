    ...
    def purchase_create_webtex(self):
        ...
        try:                
            ...
            for product_id_obj in product_obj.browse(active_ids):
                ...
                if supplier_ids:
                    prod_supp.append({'partner_id':supplier_ids[0].name,'product_id':product_id_obj,'product_qty':supplier_ids[0].qty,'supplierinfo':supplier_ids[0]})
                else:
                    prod_supp.append({'partner_id':self.partner_id,'product_id':product_id_obj,'product_qty':1,'supplierinfo':None})
            ...
            if min_stock or min_stock_sv:
                if x_min_stock>0:
                    if  virtual_available<x_min_stock:
                        if x_multi_stock>0:
                            x_rio_stock=float_round(x_multi_stock , precision_rounding=product.uom_id.rounding)    
                        else:
...
class account_invoice_tax(models.Model):
    _inherit = 'account.invoice.tax' 
                            ...
                            #_name = 'openerpmodel' = "account.invoice.tax"
    ...
    def compute(self, invoice):
        ...
        for line in invoice.invoice_line:
            ...
            result_tax = line.invoice_line_tax_id.compute_all(
                (line.price_unit * (1 - (line.discount or 0.0) / 100.0)),
                line.quantity, line.product_id, invoice.partner_id)
            
            #print 'taxes_compute',taxes
            #print 'result_tax',result_tax
            for tax in taxes:
                #print 'tax_compute',tax
                val = {
        ...
        for sale_id_obj in self.browse(cr, uid, ids, context):
            #for invoice_id_obj in sale_ids_obj.invoice_ids:
                ...
                for invoice_id_obj in sale_id_obj.invoice_ids:
                    ...
                    elif invoice_id_obj.state  == 'paid' and invoice_id_obj.payment_ids[0].journal_id.name.lower().find('contrassegno')>=0:
                        state='Contrassegno'
                    elif invoice_id_obj.state  == 'paid' and invoice_id_obj.payment_ids[0].journal_id.name.lower().find('contrassegno')<0:
                        state='Pagato'             
                    else:
    ...
    def _state_picking(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for sale_id_obj in self.browse(cr, uid, ids, context):
            ...
            if sale_id_obj.x_picking_state!=res[sale_id_obj.id]:
                self.write(cr,uid,sale_id_obj.id,{'x_picking_state':res[sale_id_obj.id]})
                #break
                    ...
                    for x_to_company_journal_id in journal_id_obj.x_to_company_journal_id:
                        ...
                        for move_id_obj in move_ids_obj:
                            ...
                            move_id_obj.write({'period_id':vals['period_id'],
                                               'journal_id':journal_id_obj.x_to_company_journal_id.id,
                                               'company_id':journal_id_obj.x_to_company_journal_id.company_id.id
                                               })
