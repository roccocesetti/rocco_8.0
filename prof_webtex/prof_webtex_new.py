...
class account_move_line_model(models.Model):
    _inherit = 'account.move.line'
    @api.one
    @api.depends('product_id','quantity','debit')
    def _default_x_product_id(self):
        self.x_price_unit=0
        if self.product_id and self.quantity:
            
            self.x_price_unit=self.debit/self.quantity
        
        return self.x_price_unit
    x_price_unit = x_fields.Float(string='Prezzo unitario', digits=dp.get_precision('Account'),
        store=True, readonly=True, compute='_default_x_product_id', track_visibility='always')
