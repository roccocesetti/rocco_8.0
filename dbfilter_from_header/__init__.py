...
def db_filter(dbs, httprequest=None):
    ...
    _logger.info('HTTP_X_ODOO_DBFILTER %s' % db_filter_hdr_odoo)
    
    _logger.info('HTTP_X_OPENERP_DBFILTER %s' % db_filter_hdr_openerp)
    _logger.info('dbs database %s' % dbs)
    return dbs
...
# Rocco li 11-09-2018
    ...
    _logger = logging.getLogger(__name__)
    _logger.info('monkey patching http.db_filter %s' % http.db_filter)
