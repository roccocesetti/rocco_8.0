...
class CustomerImportMapper(ImportMapper):
    ...
    def backend_id(self, record):
        result={'backend_id': self.backend_record.id}
        result.update({
        ...
        if record['customer'].get('role',None):
                                 if record['customer'].get('role',None)=='wholesale_customer':
                                     result.update({'property_product_pricelist':self.backend_record.pricelist_id.id })
