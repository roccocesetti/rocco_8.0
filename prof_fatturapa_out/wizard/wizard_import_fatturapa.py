...
from openerp import models, fields as x_fields, api
...
class withholding_tax(models.Model):
    ...
    @api.multi    
    def compute_amount_due(self, amount_invoice, invoice_id=None):
        ...
        if not amount_invoice and invoice_id:
            ...
            total_withholding_tax_excluded = invoice.compute_amount_withholding_excluded()
            amount_invoice = (
...
class fatturapa_fatForSDINote(orm.TransientModel):
    _name = "wizard.import.note"
    _description = "fattura pa nota rifiuto fatForSDINote"
    _columns = {
        'name':fields.char(string='Nota rifiuto', size=64, required=True),
                    }

    def rifiuto_fattura(self, cr, uid, ids,context=None):
        attach_obj=self.pool.get('fatturapa.attachment.in')
        if not context:
            context = {}
        active_ids=context.get('active_ids',[])
        if context.get('active_model','account.invoice')=='account.invoice':
            for invoice in self.pool.get('account.invoice').browse(cr,uid,active_ids,context=context):
                    if invoice.fatturapa_attachment_in_id:
                        attach=invoice.fatturapa_attachment_in_id
                        if attach.url_id.type=="json":
                                        attach_obj.json_update_request(cr,uid,[attach.id],False,
                                metadata={'olv:fatForSDINote':self.browse(cr,uid,ids[0],context=context).name},
                                context=context)

        else:
            for attach in self.pool.get('fatturapa.attachment.in').browse(cr,uid,active_ids,context=context):
                if attach.url_id.type=="json":
                                attach_obj.json_update_request(cr,uid,[attach.id],False,
                        metadata={'olv:fatForSDINote':self.browse(cr,uid,ids[0],context=context).name},
                        context=context)
    def accetta_fattura(self, cr, uid, ids,context=None):
        attach_obj=self.pool.get('fatturapa.attachment.in')
        if not context:
            context = {}
        active_ids=context.get('active_ids',[])
        if context.get('active_model','account.invoice')=='account.invoice':
            for invoice in self.pool.get('account.invoice').browse(cr,uid,active_ids,context=context):
                    if invoice.fatturapa_attachment_in_id:
                        attach=invoice.fatturapa_attachment_in_id
                        if attach.url_id.type=="json":
                                        attach_obj.json_update_request(cr,uid,[attach.id],True,
                                metadata={'olv:fatForSDINote':None},
                                context=context)
...
class WizardImportFatturapa(orm.TransientModel):
    ...
    def saveAttachment(self, cr, uid,stato_rx,fatturapa, context=None):
        #res=super(WizardImportFatturapa, self).saveAttachment(cr,uid,context=context)
        #if res:
        #    return res
        print 'saveAttachment',stato_rx
        if context is None:
            context = {}

        if stato_rx['type']=='json':
            number = stato_rx['olv:fatForSDINumFattura']
            data = stato_rx['olv:fatForSDIDataEmissione']
            fatForSDIId = stato_rx['olv:fatForSDIId']
        else:
            number=stato_rx['soap_numfat']
            data = stato_rx['InvoiceDate']
            fatForSDIId = stato_rx['id_soap']
        ...
        attach_obj = self.pool['fatturapa.attachment.in']
        attach_vals = {
            'name': '%s_%s_%s_%s.xml' % (company.vat, str(number),data,fatForSDIId),
            'datas_fname': '%s_%s.xml' % (company.vat, str(number)),
            'datas': base64.encodestring(fatturapa),
        }
        if stato_rx['type']=='json':
            attach_vals.update({
            'json_fatForSDIId': stato_rx['olv:fatForSDIId'] ,
            'log': stato_rx['description'] ,
            'stato_rx': stato_rx['returnCode'] ,
 
                })
        else:
           attach_vals.update({
            'id_soap': stato_rx['id_soap'] , 
            'stato_rx': stato_rx['stato_rx'] ,
                })
        attach_ids= attach_obj.search(cr,uid,[('name','=',attach_vals['name'])],context=context)
        if attach_ids:
    ...
    def check_partner_base_data(
        ...
        if (
            ...
            if context.get('inconsistencies'):
                ...
                context['inconsistencies']=''
            context['inconsistencies'] += (_("DatiAnagrafici.Anagrafica.Denominazione contains \"%s\"."" Your System contains \"%s\""
              )
    ...
    def getPartnerBase(self, cr, uid, DatiAnagrafici, context=None):
        ...
        if vat=='IT03863660167':
            print vat
        if vat=='IT03619120169':
            print vat
    ...
    def _prepareInvoiceLine(
        ...
        account_tax_kind_model =  self.pool['account.tax.kind']
        ...
        if float(line.AliquotaIVA) == 0.0 and line.Natura:
            account_tax_kind_ids=account_tax_kind_model.search(cr,uid,[('code','=',line.Natura)], context=context)
            if not account_tax_kind_ids:
                raise orm.except_orm(
                    _('Error!'),
                    _('tabel nature '
                      '%s and nature %s not found')
                    % (line.AliquotaIVA, line.Natura))
            account_tax_ids = account_tax_model.search(
                cr, uid,
                [
                    ('type_tax_use', 'in', ('purchase', 'all')),
                    ('kind_id', '=', account_tax_kind_ids),
                    ('amount', '=', 0.0),
                ], context=context)
    ...
    def _prepare_generic_line_data(self, cr,uid,line,context=None):
        retLine = {}
        account_tax_model = self.pool.get('account.tax')
         
        # check if a default tax exists and generate def_purchase_tax object
        ir_values = self.pool.get('ir.values')
        company_id = self.pool.get('res.company')._company_default_get(cr,uid,
            'account.invoice.line',context=context)
        supplier_taxes_ids = ir_values.get_default(cr,uid,
            'product.product', 'supplier_taxes_id', company_id=company_id)
        def_purchase_tax = False
        if supplier_taxes_ids:
            def_purchase_tax = account_tax_model.browse(cr,uid,supplier_taxes_ids,context=context)[0]
        print 'def_purchase_tax',def_purchase_tax,line.AliquotaIVA,line.Natura
        if float(line.AliquotaIVA) == 0.0 and line.Natura:
            ...
            account_taxes=account_tax_model.browse(cr,uid,account_taxes,context=context)
            ...
            account_taxes = account_tax_model.search(cr,uid,
                [
            ...
            print 'account_taxes_01',account_taxes,line.AliquotaIVA
            account_taxes=account_tax_model.browse(cr,uid,account_taxes,context=context)
            # check if there are multiple taxes with
            # same percentage
            print 'account_taxes_02',account_taxes,line.AliquotaIVA
            if len(account_taxes) > 1:
                # just logging because this is an usual case: see split payment
                _logger.warning(_(
                    "Line '%s': Too many taxes with percentage equals "
                    "to \"%s\"\nfix it if required"
                ) % (line.RiferimentoAmministrazione, line.AliquotaIVA))
                # if there are multiple taxes with same percentage
                # and there is a default tax with this percentage,
                # set taxes list equal to supplier_taxes_id, loaded before
                if (
        ...
        if account_taxes:
            print 'invoice_line_tax_ids',[(6, 0, [account_taxes[0].id])]
    ...
    def adjust_accounting_data(self,cr,uid, product, line_vals,context=None):
        if product.product_tmpl_id.property_account_expense:
            line_vals['account_id'] = product.product_tmpl_id.property_account_expense.id
        elif (product.product_tmpl_id.categ_id.property_account_expense_categ):
            line_vals['account_id'] = product.product_tmpl_id.categ_id.property_account_expense_categ.id
        account = self.pool['account.account'].browse(cr,uid,line_vals['account_id'],context=context)
        new_tax = None
        ...
        if new_tax:
            line_tax_id = line_vals.get('invoice_line_tax_ids') and line_vals['invoice_line_tax_ids'][0][2][0]
            
            line_tax = self.pool['account.tax'].browse(cr,uid,line_tax_id,context=context)
    ...
    def _computeDiscount(
        ...
        if float(DettaglioLinea.Quantita)>0:
            line_unit = line_total / float(DettaglioLinea.Quantita)
        else:
            line_unit=0.0
        if float(DettaglioLinea.PrezzoUnitario)>0:
            discount = (
    ...
    def get_purchase_journal(self, cr, uid, company, context=None):
        ...
        journal_ids = journal_model.search(
            ...
            order='id ASC',limit=1, context=context)
        if not journal_ids:
            raise orm.except_orm(
                _('Error!'),
                _(
    ...
    def invoiceCreate(
        ...
        # 2.1.1.5
        print invoice_data['supplier_invoice_number'],invoice_data['date_invoice'],partner_id
        exit_invoice_ids=invoice_model.search(cr,uid,[('partner_id','=',partner_id),
                                                      ('supplier_invoice_number','=',invoice_data['supplier_invoice_number']),
                                                      ('date_invoice','=',invoice_data['date_invoice']),
                                                      ],context=context)
        if exit_invoice_ids:
            invoice_model.write(
                cr, uid, exit_invoice_ids[0], {'fatturapa_attachment_in_id': fatturapa_attachment.id}, context=context)
            
            return exit_invoice_ids[0]
        ...
        for Withholding in Withholdings:
            ...
            invoice_data['ftpa_withholding_payment_reason'] = Withholding.\
                CausalePagamento
            if Withholding:
                        invoice_data.update({'withholding_tax':True})
            wt_found = None
            wts = None
            if Withholding:
                cau_pag_ids = self.pool['causale.pagamento'].search(cr,uid,[
                    ('code', '=',
                     Withholding.CausalePagamento)
                ],context=context)
                if cau_pag_ids:
                    wts = self.pool['withholding.tax'].search(cr,uid,[
                ...
                wt_found = False
                for wt in self.pool['withholding.tax'].browse(cr,uid,wts,context=context):
        ...
        if Stamps:
            ...
            invoice_data['stamp_amount'] = float(Stamps.ImportoBollo or '0')
        ...
        if Walfares:
            for walfareLine in Walfares:
                ...
                line_vals = self._prepare_generic_line_data(cr,uid,walfareLine,context=context)
                line_vals.update({
                    'name': _(
                    ...
                    'account_id': credit_account_id,
                    'withholding_tax_exclude':True
                })
                if company.cassa_previdenziale_product_id:
                    cassa_previdenziale_product = company.cassa_previdenziale_product_id
                    ...
                    self.adjust_accounting_data(cr,uid,
                        cassa_previdenziale_product, line_vals,context=context
                    )
                self.pool['account.invoice.line'].create(cr,uid,line_vals,context=context)
        # 2.1.1.8
        for Withholding in Withholdings:
                if  wt_found:
                    invoice_model.button_reset_taxes(cr,uid,invoice_id,context=context)
                    compute_taxes=self.pool['account.invoice.tax'].compute(cr, uid, invoice_id, context=context)
                    inv_whtax=self.pool['account.invoice.withholding.tax'].create(cr,uid,{
                        'base':(float(Withholding.ImportoRitenuta) *100) /float(Withholding.AliquotaRitenuta),
    ...
    def check_invoice_amount(
        self, cr, uid, invoice_id, FatturaElettronicaBody, context=None
    ):
        if context is None:
            context = {}
        invoice_obj=self.pool.get('account.invoice')    
        invoice_obj.write(cr, uid, invoice_id.id,
            {
    ...
    def remove_xades_sign(self, xml):
        removestr='xmlns:pfx6="http://www.tibco.com/ns/no_namespace_schema_location/NBS_FAT_WRITER/Schemas/XSD riepilogo_fattura_ordinaria_v1.2_mod.xsd"'
        removestr_2='xmlns:ns2="http://www.w3.org/2000/09/xmldsig#"'
        removestr_3='xmlns:ds="http://www.w3.org/2000/09/xmldsig#"'
        recovering_parser = etree.XMLParser(recover=True)
        root = etree.XML(xml, parser=recovering_parser)
        #root = etree.XML(xml.replace(removestr,'').replace(removestr_2,'').replace(removestr_3,''))
        for elem in root.iter('*'):
    ...
    def importfatturapa2(self, cr, uid, ids, context=None):
        if not context:
            context = {}
        context['inconsistencies'] = ''
        ...
        if url_id_obj.type=='json':
            resp=fatturapa_attachment_obj.json_imp_request(cr,uid,ids=ids,processid='1',context=context)
        else:
            resp=fatturapa_attachment_obj.soap_imp_request(cr,uid,ids=[],context=context)
            
        context['active_ids']=resp['import_ids']
        if resp:
            res_fpa= self.importFatturaPA(cr,uid,ids,context=context)
            return res_fpa
        else:
            return {
            'view_type': 'form',
    ...
    def importFatturaPA(self, cr, uid, ids, context=None):
        ...
        new_invoices = []
        fatturapa_attachment_obj = pool.get('fatturapa.attachment.in')
        fatturapa_attachment_ids = context.get('active_ids', False)
        invoice_model = pool.get('account.invoice')
        for fatturapa_attachment_id in fatturapa_attachment_ids:
            ...
            if fatturapa_attachment:
                try:
                    ...
                    ok=True
            # decrypt  p7m file
            #fatturapa_attachment=fatturapa_attachment.ir_attachment_id
            ...
            try:
                ...
                print 'xml_string',xml_string
                if xml_string.find('<?xml version="1.0" encoding="UTF-8"?>')<0:
